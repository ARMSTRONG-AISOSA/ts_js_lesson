...
// Example 1:
// ✅ Explanation:
// Generator<Y, R, N> is the TypeScript type for generator functions:
//      Y → Type of values yielded (here number).
//      R → Type of the return value (here void, since the generator doesn’t return anything).
//      N → Type of values that can be passed back into the generator via next(value) (here unknown since we don’t use it).
function* getNumbers(): Generator<number, void, unknown> {
    yield 1;
    yield 2;
    yield 3;
}
...
function* getValues(): Generator<string | number, void, unknown> {
    yield "Position 1 0f 3";
    yield "Position 2 0f 3";
    yield "Position 3 0f 3";
};

let values: Generator<string | number, void, unknown> = getValues();
let result: IteratorResult<string | number, void> = values.next(); // start things off
...
function* startUp(): Generator<1 | 2 | 3, number, unknown> {
    yield 1;
    yield 2;
    yield 3;
    return 4;
