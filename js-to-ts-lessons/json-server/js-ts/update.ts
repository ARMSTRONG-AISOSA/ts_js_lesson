const blogUpdateForm = document.querySelector('#blogUpdateForm') as HTMLFormElement | null;

// Input Fields
const titleField = document.querySelector('#titleUpdate') as HTMLInputElement | null;
const authorField = document.querySelector('#authorUpdate') as HTMLInputElement | null;
const categoryField = document.querySelector('#categoryUpdate') as HTMLInputElement | null;
const tagsField = document.querySelector('#tagsUpdate') as HTMLInputElement | null;
const contentField = document.querySelector('#contentUpdate') as HTMLTextAreaElement | null;

// Button
const homeBtn = document.querySelector('#returnHome') as HTMLButtonElement | null;

// .get('id')	Retrieves the value of id from the query string
const paramsUpdate = new URLSearchParams(window.location.search);
const postIdUpdate: string | null = paramsUpdate.get("id");


// Urls
let baseURLUpdate: string = "http://localhost:3005/posts/";
let homeURLUpdate: string = "http://127.0.0.1:8080/";
...
interface Post {
    ...
    postUpdatedLast?: string | undefined;
    tags?: string[];
}
...
async function fetchSinglePostUpdate(): Promise<void> {
    let template = '';
    ...
    try {
        const response = await fetch(baseURLUpdate + `${postIdUpdate}`); //fetch data

        if (!response.ok) {
            throw new Error(`Failed to fetch post. Status: ${response.status}`);
        }

        const post: Post = await response.json(); // Parse data
...
if (blogUpdateForm) {
    ...
    blogUpdateForm.addEventListener("submit", async (event: SubmitEvent): Promise<void> => {
        ...
        if (titleField instanceof HTMLInputElement &&
            ...
            const response = await fetch(baseURLUpdate + postIdUpdate, {
...
window.addEventListener("DOMContentLoaded", async function fetchPost(): Promise<void> {
    const post = await fetchSinglePostUpdate();
})