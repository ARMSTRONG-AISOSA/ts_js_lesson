...
function delayedValue(value: string, ms: number): Promise<string> {
...
// : Promise<string>[] => “an array of Promises that each resolve to a string”.
...
async function* dataFetcher(urls: string[]): AsyncGenerator<string[] | string | unknown > {
...
async function getSequentialData() {
    ...
    const fetcher: AsyncGenerator<string[] | string | unknown > = dataFetcher(apiEndpoints);
...
// tsc index.ts --target es2015 --lib es2018,dom