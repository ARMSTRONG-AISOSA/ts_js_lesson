...
class MathUtil {
    ...
    // circle diameter
    static getCircleDiameter(radius: number): number {
        return radius * 2;
    }

    // circumference
    static getCumference(radius: number): number {
    ...
    // half value
    static half(value: number): number {
        return value / 2;
    }
}
...
class Config {
    // Private static field MUST be explicitly typed in TypeScript
    static readonly #defaultSettings: { max: number; min: number; theme: string } = {
        max: 10,
        min: 1,
    ...
    static {
        ...
        if (isDevelopment) {
            ...
            // Safer assignment for production => this.class & this.property
